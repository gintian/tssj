...
export default {
  ...
  mounted () {
    ...
    if(this.isBird.is){
     ...
     this.$httpWrapPOST('/video/rtspAgent',{
          "param":{
            "url":this.videoSrc
          }
        })
  ...
  methods: {
   ...
   //   console.log('player',this.$refs.player.offsetWidth)
    ...
    runPtz(name){
      ...
      this.$httpWrapPOST('/ship/startPTZ',{
          param:{
    ...
    stopPtz(name){
      ...
      this.$httpWrapPOST('/ship/stopPTZ',{
          param:{
            cameraId:this.videoID,
            dwPTZCommand:obj[name],
          }  
    ...
    pTZControl(command,action,indexCode){
      this.$httpWrapPOST('/artemis/pTZControl',{
        param:{
    ...
    initWebSocket(){ //初始化weosocket
      const wsuri = "ws://10.25.19.149/zfw/websocket";
