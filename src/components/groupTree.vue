<template>
  <div class="custom-tree-container">
        ...
        <el-table :data="tableData" style="width: 100%" :header-cell-style="tableHeaderColor">
          ...
          <el-table-column label="海域点位" prop="points" width="160">
            <template slot-scope="props">
              <span
                v-for="real in props.row.points"
                :key="real.index"
              >纬度：{{real.lat}},经度：{{real.lon}}；</span>
            </template>
          </el-table-column>
          <el-table-column key="7" prop="opr" label="操作" align="center">
            <template slot-scope="scope">
              <el-button
                ...
                size="small"
                class="btn-upt"
                @click="handleUpdate(scope.row,index)"
                v-show="tableData[scope.row.id].show"
              >导入</el-button>
    ...
    ></el-tree>
    <el-dialog :title="areaInfo.label" :visible.sync="areaInfo.dialog" width="15%" :modal="false">
    ...
    <el-dialog
      ...
      :show-close="false"
    >
      <el-input type="textarea" :rows="2" placeholder="请输入内容" v-model="focusDialog.des"></el-input>
  ...
  methods: {
      ...
      for (let i in this.tableData) {
        ...
        if (this.tableData[i].show) {
          ...
          let [level, type, name, points, radius, lat, lon] = [
            ...
            this.tableData[i].points,
            this.tableData[i].radius,
            this.tableData[i].lat,
            this.tableData[i].lon
          ];
          this.service
            .post("/water/save", {
              level: level,
    ...
    renderContent(h, { node, data, store }) {
      return (
        <span class="custom-tree-node" style="width:100%">
          ...
          <span
            class="tree_node_top"
            on-click={ev => this.moverView(ev, store, data)}
          >
            {" "}
    ...
    labelOverlay(data, label) {
      ...
      ComplexCustomOverlay.prototype.draw = function() {
        ...
        if (_this.map.getZoom() > 10 || _this.map.getZoom() < 9) {
          /*地图缩放到9级以下 对文字隐藏*/ this._div.style.height = "0px";
    ...
    focusArea(ev, store, data) /*关注区域*/ {
      ...
      if (data.isFocus) {
        this.$emit("isFocus", {
          description: this.focusDialog.des,
          focus: true,
          targetType: 8,
          targetSign: this.checkItem.id
        });
        data.isFocus = false;
      } else {
    ...
    addMarker(marker, data) {
      ...
      const mp = {
        ...
        circle: () => {
          let bd09Arr = wgs84ToBD(
            parseFloat(marker.centerx),
            parseFloat(marker.centery)
          );
          let points = [];
        ...
        rectangle: () => {
          let bd09Arr = wgs84ToBD(
            parseFloat(marker.lon1),
            parseFloat(marker.lat1)
          );
          let bd09Arr2 = wgs84ToBD(
            parseFloat(marker.lon2),
            parseFloat(marker.lat2)
          );
          let points = [];
          points.push(new BMap.Point(bd09Arr[0], bd09Arr[1]));
