<template>
  <div class="video-player-mian" :style="{width:width,'max-width':maxWidth }">
    <!-- 'max-width':maxWidth -->
    ...
    <svg
      v-show="!isLoadIn&&aiBoxShow&&rtsp===currentMakers.rtsp"
      :width="myVideoWidth"
      :height="myVideoHeight+75"
    ...
    >
      <circle
        @click="test"
        :cx="parseInt( ( myVideoWidth / parseInt(currentMakers.videoWidth) ) * parseInt(value.list[0].x))"
        :cy="parseInt( ( myVideoWidth / parseInt(currentMakers.videoWidth) ) * parseInt(value.list[0].y))"
        :r="value.radius"
        v-for="(value,index) in getShapesArrForType(0)"
        :key="index+4"
        :style="{stroke:value.color, fill:parseInt(value.size)<0?value.color:'rgba(255, 0, 255, 0)', 'stroke-width': value.size}"
      />
      <polygon
        v-for="(value,index) in getShapesArrForType(1)"
        :key="index+1"
        :points="getPolygonRectangle(currentMakers.videoWidth)(value.x1,value.y1,value.x2,value.y2)"
        :style="{stroke:value.color, fill:parseInt(value.size)<0?value.color:'rgba(255, 0, 255, 0)', 'stroke-width': value.size}"
      />
      <polyline
        v-for="(value,index) in getShapesArrForType(2)"
        :key="index+2"
      ...
      <polygon
        v-for="(value,index) in getShapesArrForType(3)"
        :key="index+3"
        :points="getPolygonPoints(value.list)"
        :style="{stroke:value.color, fill:value.size<0?value.color:'rgba(255, 0, 255, 0)', 'stroke-width': value.size}"
      />
      ...
      <text
        :x="parseInt( ( myVideoWidth / parseInt(currentMakers.videoWidth) ) * parseInt(value.x))"
        v-for="(value,index) in getShapesArrForType(4)"
        :key="index+5"
      ...
      <svg v-for="(value,index) in getShapesArrForType(100)" :key="index">
        <polyline
          v-for=" (item,myIndex) in getPeoplePoints(value.list)"
          :key="myIndex"
          :points="item"
          :style="{stroke:value.color, fill:value.size<0?value.color:'rgba(255, 0, 255, 0)', 'stroke-width': value.size}"
        />
      ...
      if (flvjs.isSupported()) {
        ...
        if (video) {
          ...
          try {
            ...
            this.player.on(
              flvjs.ErrorDetails.MEDIA_CODEC_UNSUPPORTED,
              (errType, errDetail) => {
                console.log("视频错误");
              }
            );
    ...
    getNumsText() {
      ...
      this.currentMakers.shapes.forEach(e => {
        ...
        textArr.push({
          ...
          nums: e.nums,
          color: e.color,
          x: parseInt(parseInt(e.list ? e.list[0].x : e.x1 ? e.x1 : e.x) * sim),
          y: parseInt(parseInt(e.list ? e.list[0].y : e.y1 ? e.y1 : e.y) * sim)
        });
        i++;
      });
      return textArr;
...
.menu-video-control-btn-box {
  ...
  padding: 0 10px;
  height: 50px;
  background-image: linear-gradient(rgba(238, 238, 238, 0), #222);
}
.menu-video-btn-list {
  height: calc(100% - 4px);
  display: grid;
