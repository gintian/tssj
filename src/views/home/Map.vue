<template>
  <div id="home" style="z-index: 900;">
    ...
    <el-button
      @click="loadVideo"
      type="warning"
      style="position: absolute;top: 8%;z-index: 400;right: 9%; box-shadow: rgb(217 163 50) 4px 4px 10px;"
    >视频</el-button>
    <!-- 统计图标 -->
    <button
      class="signTitle"
      style="position: absolute;top: 59%;z-index: 400;right: 8px;width:23px;height:25px;"
    ...
    <div
      v-if="myMovingMarker"
      id="myMovingMarker"
      style=" position: absolute;left:10%; bottom:20%;z-index: 400;"
    >
    ...
    <div
      id="import"
      style=" position: absolute;right:6%; bottom:35%;z-index: 400;"
      v-drag
      v-show="showimportexcel"
    >
    ...
    <div id="mapControl" style=" position: absolute;right: 10px; bottom:24%;z-index: 400;">
      <mapControl
        v-model="mapZoom"
        @add="map.setZoom(map.getZoom()+1)"
        @minus="map.setZoom(map.getZoom()-1)"
   ...
   <!-- 层级、区域船舶显示 -->
    ...
    >
      <Dropdown
        v-for="(item,index) in dropdownTitleData"
        @iconClick="dropdownButton"
        :item="item"
        :key="index"
        :title="item.title"
        :titleIcon="item.icon"
        :dropdownData="item.dropdownInfoData"
        ref="dropDown"
      />
    </div>
    ...
    <div
      id="selectMarker"
      style=" position: absolute;right: 3%; top:9%;z-index: 400"
      v-drag
      v-show="showObjectSelect"
    >
      <ObjectSelect
        @ObjectSelect="ObjectSelect"
        @hide="showObjectSelect=false"
    ...
    <div
      id="selectMarker3"
      style=" position: absolute;right: 5%; top:20%;z-index: 400;width: 350px"
      v-drag
      v-show="showSelectMarker"
    >
      <SelectMarker
        @selectMarker="selectMarker"
        @hide="showSelectMarker=false"
    ...
    <div
      ...
      id="ShipDataInfo"
      v-drag
      v-show="showInfo.ship"
    >
      <ShipInfoView
        :shipTabObj="dialogInfo.ship"
    ...
    <!-- 船只详细信息 -->
    <div
      style="height: auto;width: auto;position: absolute;left: 26%; top:15%;z-index: 500;"
      id="ShipDetail"
      v-drag
      v-show="showInfo.shipDetail"
    >
      <ShipDetailView :shipTabObj="dialogInfo.shipDetail" @remove="showInfo.shipDetail=false" />
    </div>

    <!-- 船舶跟踪轨迹时间选择 -->
    <div
      style="height: auto;width: auto;position: absolute;right: 10%; top:45%;z-index: 500;"
      v-drag
    ...
    <!-- radar表格信息 -->
    <div
      class="radarShipDataInfo"
      style="height: auto;width: auto;position: absolute;left: 23%; top:10%;z-index: 500"
      v-drag
      v-show="showInfo.radarShip"
    >
      <RadarDataTabView
    ...
    <div
      id="pierInfoView"
      style="height: auto;width: 550px;position: absolute;left: 23%; top:10%;z-index: 500"
      v-drag
      v-show="showInfo.berth"
    >
      <PierInfoView
    ...
    <div
      ...
      style="height: auto;width: 400px;position: absolute;left: 23%; top:10%;z-index: 500"
      v-drag
      v-show="showInfo.navy"
    >
      <NavyInfoView :tabObj="dialogInfo.navy" titleName @remove="showInfo.navy=false" />
    </div>
    <!-- 海警局信息框 -->
    <div
      id="MarpoInfoView"
      style="height: auto;width: 400px;position: absolute;left: 23%; top:10%;z-index: 500"
      v-drag
      v-show="showInfo.mary"
    >
      <MarpoInfoView :tabObj="dialogInfo.mary" titleName @remove="showInfo.mary=false" />
    </div>
    <!-- 军分区信息框 -->
    <div
      id="MilidivisInfoView"
      style="height: auto;width: 400px;position: absolute;left: 23%; top:10%;z-index: 500"
      v-drag
      v-show="showInfo.milidivi"
    >
      <MilidivisInfoView :tabObj="dialogInfo.milidivi" titleName @remove="showInfo.milidivi=false" />
    </div>
    <!-- 海事局信息框 -->
    <div
      id="MSA"
      ...
      v-show="showInfo.msa"
    >
      <MSA :tabObj="dialogInfo.msa" titleName @remove="showInfo.msa=false" />
    </div>
    <!-- 海事局信息框 -->
    <div
      id="PSB"
      ...
      v-show="showInfo.psb"
    >
      <PSB :tabObj="dialogInfo.psb" titleName @remove="showInfo.psb=false" />
    </div>
    <!-- 军民融合办信息框 -->
    <div
      id="CMIO"
    ...
    >
      <CMIO :tabObj="dialogInfo.cmio" titleName @remove="showInfo.cmio=false" />
    </div>
    <!-- 海洋与渔业局信息框 -->
    <div
      id="OFA"
      ...
      v-show="showInfo.ofa"
    >
      <OFA :tabObj="dialogInfo.ofa" titleName @remove="showInfo.ofa=false" />
    </div>
    <!-- 边检信息框 -->
    <div
      id="FI"
      ...
      v-show="showInfo.fi"
    >
      <FI :tabObj="dialogInfo.fi" titleName @remove="showInfo.fi=false" />
    </div>
    <!-- 海关信息框 -->
    <div
      id="customs"
    ...
    >
      <Customs :tabObj="dialogInfo.customs" titleName @remove="showInfo.customs=false" />
    </div>
    <!-- 海底光缆 信息框 -->
    <div
      id="seaLineInfoView"
    ...
    >
      <VideoPlayer
        v-for="(itme,index) in dialogInfo.camera"
        :key="index"
    ...
    <div
      id="dailyEventDiag"
      v-drag
      v-show="showDailyEvent"
      style="height: auto;width: auto;position: absolute;left: 23%; top:10%;z-index: 500"
    >
      <DailyEventDiag
        @hide="showDailyEvent=false"
        :dailyList="dailyEventList"
        @change1="changeDailyEvent"
        @search="searchDailyEvent"
      ></DailyEventDiag>
    </div>
    ...
    <div
      id="focusDialog"
      v-drag
      v-show="showInfo.focus"
      style="height: auto;width: auto;position: absolute;left:5%; top:10%;z-index: 500"
    >
      <FocusDialog
        @hide="showInfo.focus=false"
        :table-data="dialogInfo.focus"
        @rowClick="focusClick"
      ></FocusDialog>
    </div>
    <!--            区域分组显示-->
    <div
      id="groupView"
      ref="groupView"
      v-show="showGroupView"
      style="position: absolute;right: 10%; top:20%;z-index: 500"
      v-drag
    >
        ...
        >
          <div class="drawItem">
            ...
            <p
              ...
              :key="index"
              :class="{'acitve':index===isDrawType}"
              @click="drawArea(item.value,index)"
            >{{item.label}}</p>
          </div>
          <el-row>
            <el-col :span="15" style="margin-left:10px;">
              <el-form-item label="名称:" prop="name" label-width="60px" class="name-item">
                <el-input class="nameInput" v-model="drawData.name" ref="nameInput" size="medium " />
              </el-form-item>
            </el-col>
          </el-row>
          <!-- <el-row>
          ...
          <div class="drawItem" v-show="drawData.positions[0]">
            <span>
              +新增点
              <el-button
                icon="el-icon-edit"
            ...
            <div class="drawRadius drawPos" v-if="isDrawType===0">
              <el-form-item label label-width="0" prop="radius">
                <input v-model="drawData.radius" :disabled="disabledDraw" />
                <p>半径</p>
              </el-form-item>
            </div>
            <div class="drawPos" v-for="(pos,index) in drawData.positions" :key="index">
              <span>{{index+1}}.</span>
              <input v-model="pos[0]" :disabled="disabledDraw" />
              <p>经</p>
              <input v-model="pos[1]" :disabled="disabledDraw" />
              <p>纬</p>
            </div>
          </div>
          <div class="drawItem">
  ...
  methods: {
    ...
    queryArea() {
      ...
      for (let i in this.objectArea) {
        ...
        let [radius, lat, lon, type] = [
          this.objectArea[i].radius,
          this.objectArea[i].lat,
          this.objectArea[i].lon,
          this.objectArea[i].type
        ];
        this.service
          .post("/ship/screeningWater", {
            // type: area,
            type: type,
            lat: lat,
            lon: lon,
...
.DrawMain /deep/ .el-input__inner,
.DrawMain /deep/ .el-form-item__label,
.DrawMain /deep/ .el-textarea__inner {
  background: transparent;
  color: black;
}
